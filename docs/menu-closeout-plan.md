...
| 53 | **Go and organizer:** dynamic Locations, Smart Folders/Recents and Hashtags menu groups; create/edit/delete saved queries, plus bounded automatic tag rescan for small local roots. | Rename/move/delete/external edits update queries/tags when watching succeeds; inaccessible or large roots fall back to a clearly marked manual refresh; no stale results presented as current. | 50 shared actions |
...
The original plan has **12 cycles, 48–59**. Cycle 59 reached an integrated acceptance checkpoint for the implemented subset; it did **not** close exact iA parity. Later Cycles 54b–54c added bounded, independent opt-in Smart Quotes and Smart Dashes with 101 passing tests and native typing/Undo evidence. The remaining work is tracked as explicit partial rows in the [acceptance ledger](release-acceptance.md) and [menu audit](ia-menu-audit-2026-09-23.md), principally writing substitutions/correction, style/syntax categories, post-setup authorship, native Window/application actions, deeper Markdown/output, and systematic accessibility/state/hardware checks. No reliable final cycle count exists until the unobserved iA behavior is captured and scoped. Developer ID signing and notarization additionally require credentials and another-machine acceptance.
