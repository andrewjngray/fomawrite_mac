...
**Cycles 51–57a overlay:** Tree/List navigation and View preview routes are native-verified through Cycle 51. Cycles 52–53 add statistics/chrome and the dynamic Go menu with bounded small-root tag refresh; Cycle 54a adds manual local completions; Cycle 55 adds bounded live Custom and Fillers style checks; Cycle 56a adds the pre-setup Authors profile sheet; Cycle 57a adds bundled offline Help and What’s New pages. Current source passes 97 tests. Partial Dev checks cover the corresponding menu structure and selected safe workflows, including both Help routes and Close/Escape through accessibility. Remaining visual, accessibility and workflow checks are open. Online Support, OS Help search behavior and Window/AppKit closeout remain pending. See [cycle records](build-cycles.md).
