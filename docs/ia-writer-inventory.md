...
**Cycles 51–54a overlay:** Tree/List navigation and View preview routes are native-verified through Cycle 51. Cycles 52–53 add statistics/chrome and the dynamic Go menu with bounded small-root tag refresh; Cycle 54a adds manual local completions. Their source passes 89 tests, but native Dev verification remains pending. Edit → Substitutions children and iA completion behavior were not captured, so completion parity and automatic correction remain open. See [cycle records](build-cycles.md).
