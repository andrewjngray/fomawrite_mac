...
**Current menu audit — 24 September 2026:** Cycle 59 is an implemented-subset integrated checkpoint at code commit `5f8d64b` on macOS 27.0. Source passes 99 tests; both ordinary and Dev bundles are refreshed, local ad-hoc signed and strict-verified. Evidence across Cycles 48–58 inspected all ten top-level menu families and representative safe workflows, but not every saved/untitled/dirty state, enabled-state transition, VoiceOver, multi-display or dark/narrow condition. The consolidated [menu map and gap audit](ia-menu-audit-2026-09-23.md) and [acceptance ledger](release-acceptance.md) are the current status; dated descriptions below are historical. The [read-only native iA follow-up](../research/menu-audit-2026-09-24/native-submenus.md) supplies labels without implying behavior parity.
