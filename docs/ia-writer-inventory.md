...
**Cycles 51–58a overlay:** Tree/List navigation and View preview routes are native-verified through Cycle 51. Cycles 52–53 add statistics/chrome and the dynamic Go menu with bounded small-root tag refresh; Cycle 54a adds manual local completions; Cycle 55 adds bounded live Custom and Fillers style checks; Cycle 56a adds the pre-setup Authors profile sheet; Cycle 57a adds bundled offline Help and What’s New pages; Cycle 58a resolves heading fragments in explicit local Markdown/text links and wikilinks through source, Go → Open Link and preview routes. Current source passes 98 tests. The fragment is excluded from file identity. Native Dev opened the synthetic target cleanly with no fragment in its title; exact caret/preview jumps and dirty Cancel are covered by automation. Cross-window transfer and missing-anchor feedback remain open, and this does not claim full Markdown parser/output parity. See [cycle records](build-cycles.md).
