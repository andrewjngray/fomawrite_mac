...
**Cycle 50 overlay:** File follows the observed New/Open/Recent/Close/Save/document-operation/reveal/Share/Export/Print/Page Setup grouping while retaining explicit Omawrite Extras. Edit exposes focused-editor Copy Formatted, Copy HTML and Copy Markdown alongside focused-field core editing, Find, spelling, transformations, speech and manual authorship commands. Saved/untitled guards and dirty Close → Cancel remain intact. The hidden iA submenu children are not inferred; AppKit AutoFill/Dictation and duplicate Emoji entries remain a native-menu inspection issue, not parity evidence. [Build and native evidence](../research/cycle-50/README.md).

**Current status — 23 September 2026:** Cycle 50 is the latest local build, with 81 tests passing. Mac 0.2.0 RC1 remains the older Cycle 42 downloadable release. The [release acceptance ledger](release-acceptance.md) and [menu closeout plan](menu-closeout-plan.md) track unresolved parity and hardware checks; dated counts below are historical.
