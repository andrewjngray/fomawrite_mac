...
**Cycles 51–56a overlay:** Tree/List navigation and View preview routes are native-verified through Cycle 51. Cycles 52–53 add statistics/chrome and the dynamic Go menu with bounded small-root tag refresh; Cycle 54a adds manual local completions; Cycle 55 groups Focus controls and adds bounded live Custom and Fillers style checks; Cycle 56a adds the pre-setup Authors profile sheet. Current source passes 96 tests. Partial Dev checks cover menu structure, fixture statistics, automatic small-root tag changes, completion/Undo, both live style-check menu items and the blank Authors sheet; remaining visual, accessibility and workflow checks are open. Clichés, Redundancies and Show Syntax are absent. Edit → Substitutions children, iA completion behavior and post-setup iA authorship semantics remain uncaptured. See [cycle records](build-cycles.md).
