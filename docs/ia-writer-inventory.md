...
**Cycle 50b overlay:** Edit → Transformations now has the captured four labels in order. Capitalize matches the directly observed plain-text sample `tEST of THE wORLD` → `Test Of The World`, preserves the selection and supports one-step Undo. Build and all 102 native-access tests pass; the refreshed Dev app verified the menu, result and Undo in a disposable draft. iA's Make Title Case differed on the same sample, so Omawrite's existing Title Case algorithm remains a distinct open gap. [iA reference](../research/cycle-50/ia-capitalize-reference.txt) and [native verification](../research/cycle-50/native-capitalize.txt).
