...
**Cycles 51–56a overlay:** Tree/List navigation and View preview routes are native-verified through Cycle 51. Cycles 52–53 add statistics/chrome and the dynamic Go menu with bounded small-root tag refresh; Cycle 54a adds manual local completions; Cycle 55 groups Focus controls and adds the bounded live Custom style check; Cycle 56a adds the pre-setup Authors profile sheet. Current source passes 94 tests. Partial Dev checks cover menu structure, fixture statistics, automatic small-root tag changes, completion/Undo, Custom menu presence and the blank Authors sheet; remaining visual, accessibility and workflow checks are open. Edit → Substitutions children, iA completion behavior and post-setup iA authorship semantics remain uncaptured. See [cycle records](build-cycles.md).
...
Omawrite has sentence/paragraph focus and independent typewriter scrolling under the captured Focus hierarchy. Cycle 55b adds one opt-in live style category: Custom terms entered in Writing Review. Matching is bounded, excludes code and URL destinations, and changes highlighting without changing Markdown or Undo state. Fillers, Clichés, Redundancies, linguistic syntax categories and provenance visibility remain planned separately. “Show Markdown syntax” in Omawrite means visible markup, not iA's parts-of-speech coloring.
