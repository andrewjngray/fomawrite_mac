...
**Current menu audit — 24 September 2026:** Cycle 59 is an implemented-subset integrated checkpoint at code commit `5f8d64b` on macOS 27.0. At the Cycle 59 checkpoint, source passed 99 tests; both ordinary and Dev bundles were refreshed, local ad-hoc signed and strict-verified. Evidence across Cycles 48–58 inspected all ten top-level menu families and representative safe workflows, but not every saved/untitled/dirty state, enabled-state transition, VoiceOver, multi-display or dark/narrow condition. The consolidated [menu map and gap audit](ia-menu-audit-2026-09-23.md) and [acceptance ledger](release-acceptance.md) are the current status; dated descriptions below are historical. The [read-only native iA follow-up](../research/menu-audit-2026-09-24/native-submenus.md) supplies labels without implying behavior parity.
...
**Cycle 50 overlay:** File follows the observed New/Open/Recent/Close/Save/document-operation/reveal/Share/Export/Print/Page Setup grouping while retaining explicit Omawrite Extras. Edit exposes focused-editor Copy Formatted, Copy HTML and Copy Markdown alongside focused-field core editing, Find, spelling, transformations, speech and manual authorship commands. Saved/untitled guards and dirty Close → Cancel remain intact. The hidden iA submenu children are not inferred; AppKit provides AutoFill, Dictation and Emoji, and a duplicate Omawrite Emoji item was removed after native inspection. [Build and native evidence](../research/cycle-50/README.md).

**Cycles 51–58a overlay:** Tree/List navigation and View preview routes are native-verified through Cycle 51. Cycles 52–53 add statistics/chrome and the dynamic Go menu with bounded small-root tag refresh; Cycle 54a adds manual local completions; Cycle 55 adds bounded live Custom and Fillers style checks; Cycle 56a adds the pre-setup Authors profile sheet; Cycle 57 adds bundled offline Help and native Window → Center; Cycle 58a resolves heading fragments in explicit local Markdown/text links and wikilinks through source, Go → Open Link and preview routes. Current source passes 99 tests. Center was present and invoked natively without changing the target document status, while geometry remains automated only. A later live screenshot displayed the expected second heading and source caret after Go → Open Link, but no screenshot was saved and preview scroll was not confirmed. Cross-window transfer, missing-anchor feedback and OS/multi-display Center geometry remain open. See [cycle records](build-cycles.md).

**Cycle 59 overlay:** final build/test logs record 99/0/0 on macOS 27.0. Both local bundles were refreshed and strict signature-verified. Ordinary reopened saved README.md with Authors present; Dev remained clean on Fillers.md. This consolidates the implemented subset and evidence, not full menu parity. Explicit residuals remain: Cycle 54 substitutions; Cycle 55 Clichés/Redundancies/Show Syntax; Cycle 56 post-setup Mark As/Paste Edits; Cycle 57 AppKit/Application/Window actions; Cycle 58 parser/output/cross-app and cross-window fragment behavior. [Cycle 59 record](../research/cycle-59/README.md).

**Cycle 54b overlay:** Edit → Substitutions → Smart Quotes is now a persisted opt-in typing behavior. It replaces only a directly typed straight double quote and chooses opening/closing form from local punctuation context. The first Undo restores the straight quote; the second removes the literal keystroke. Paste, selected text, IME composition, code, URLs, link destinations and raw-tag contexts are excluded. This is a bounded Smart Quotes subset: Smart Dashes, Text Replacement, capitalization/correction and native input-method/layout coverage remain open. Build and all 100 native-access tests pass. In refreshed Dev, typing `"hello"` produced `“hello”`; Command-Z restored the final straight quote, and disabling the setting left new input straight. The disposable draft was discarded and the prior saved sample restored. [Cycle record](../research/cycle-54/README.md).

**Cycle 54c overlay:** Edit → Substitutions → Smart Dashes is now independently persisted and opt-in. A direct iA sample established one rule: with the option enabled, typing the second hyphen of a spaced prose `--` pair produced an em dash and one Undo restored `--`; with it disabled, the pair stayed literal. Omawrite implements exactly that rule, with the same paste, selection, IME, code, URL, link-destination and raw-tag exclusions as Smart Quotes. Markdown rules/front matter, third hyphens and unobserved unspaced forms stay literal. Other dash forms, Text Replacement, Smart Copy/Paste and correction remain open. Build and all 101 native-access tests passed at that checkpoint. Native Dev typing and Undo passed; the initially unverified disposable-draft close was later confirmed by the Cycle 50b native check, with clean Second.md visible before refresh. [Reference observation](../research/cycle-54/native-smart-dashes-reference.txt), [Omawrite native evidence](../research/cycle-54/native-smart-dashes.txt) and [cycle record](../research/cycle-54/README.md).

**Cycle 50b overlay:** Edit → Transformations now has the captured four labels in order. Capitalize matches the directly observed plain-text sample `tEST of THE wORLD` → `Test Of The World`, preserves the selection and supports one-step Undo. Build and all 102 native-access tests pass; the refreshed Dev app verified the menu, result and Undo in a disposable draft. Cycle 50c separates Make Title Case and matches two directly observed English samples; full locale/punctuation/style rules remain open. [iA references](../research/cycle-50/ia-title-case-reference.txt) and [native verification](../research/cycle-50/native-title-case.txt).
...
Cycle 56a adds the observed macOS Authors → Set Up Authorship… entry and a local persistent profile with required Name and optional Identifier. Save validates and trims the profile; Cancel preserves the previous values. Reopening permits profile editing as an explicit Omawrite choice because post-setup iA behavior was not observed. The profile does not label document text or establish provenance. Existing Edit → Authorship Annotations remains a separate manual assertion tool; Mark As and Paste Edits From semantics are not implemented.
...
Omawrite has sentence/paragraph focus and independent typewriter scrolling under the captured Focus hierarchy. Cycle 55b adds opt-in live Custom terms entered in Writing Review; Cycle 55c adds Fillers using `very`, `really`, `quite` and `just`. Both use case-insensitive Unicode whole-word matching, share the bounded offset-preserving prose mask and do not change Markdown or Undo state. An exact Custom/Fillers term overlap is labelled Custom. Clichés, Redundancies, linguistic Show Syntax categories and provenance visibility remain planned separately. “Show Markdown syntax” in Omawrite means visible markup, not iA's parts-of-speech coloring.
...
Help search, iA Writer Help, What's New, online support. Cycle 57a adds bundled offline Omawrite Help and What’s New pages in a read-only in-app Markdown viewer while retaining Keyboard Shortcuts. The pages use a fixed resource allowlist and do not touch the editor document. Online Support is absent because no verified destination exists. OS Help search behavior and remaining visual/accessibility checks are unverified.
