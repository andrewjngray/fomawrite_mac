...
## Cycle 54 — writing input (phases 54a–54b partial)
...
Source checkpoint 54b: Edit → Substitutions → Smart Quotes is a persisted, opt-in toggle. A directly typed straight double quote becomes an opening or closing curly double quote using the immediately preceding character; existing text is never scanned or rewritten. The literal keystroke and automatic replacement use separate document edits, so the first Undo restores the straight quote and the second removes it. The handler refuses selected text and IME composition, and does not run for paste. Fenced, indented and inline code, bare URLs, Markdown link destinations and incomplete raw tags retain straight input. Smart Dashes, Text Replacement, capitalization and automatic correction remain absent.

Validation: Cycle 54b `./bin/build` and the full native-access `./bin/test` suite pass with **100 tests, zero failures and zero skips**. Focused coverage checks opening/closing output, persisted checked state, the two-stage Undo sequence and code/URL/link/raw-tag refusals. The refreshed Dev app exposed Edit → Substitutions → Smart Quotes. In a disposable unsaved draft, typing `"hello"` produced `“hello”`; Command-Z restored the final straight quote, and turning Smart Quotes off left the next typed quote straight. The draft was discarded, and Dev returned to saved Second.md. Cycle 54a's earlier native pass displayed two local suggestions for `lan`; Return accepted one and Command-Z restored the prefix. [Native evidence](../research/cycle-54/native-menu.txt), [Smart Quotes evidence](../research/cycle-54/native-smart-quotes.txt) and [cycle gaps](../research/cycle-54/README.md) remain explicit.

Known gaps: candidates beyond the bounded window are omitted. Only Smart Quotes has been behavior-observed and implemented from the Substitutions group. Smart Dashes, Text Replacement, automatic correction/capitalization and AutoFill remain unimplemented. IME, paste, selection, non-US layouts, popup visuals, arrow keys, Escape, narrow/dark and accessibility remain pending native checks. Both ordinary and Dev bundles were refreshed and strict signature-verified; ordinary reopened its prior saved document and exposed the new menu. [Optional sample exercise](../research/usability/cycle-54.md): request a completion for `lan`, accept and Undo; then enable Smart Quotes, type a quoted sentence, Undo its final quote and try code/link cases.
