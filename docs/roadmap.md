...
**Current status — 24 September 2026:** Cycle 51 is the latest fully native-verified build. Current Cycles 52–56 source pass 96 tests; the ordinary app is packaged from Cycle 53 and Dev is refreshed with Cycle 55c/56a source. Partial native checks cover View/Go/Focus menus, fixture statistics, automatic small-root tags, completion/Undo, Custom and Fillers menu presence, and the pre-setup Authors sheet. Fillers is a persisted live overlay for the starter words `very`, `really`, `quite` and `just`; Clichés, Redundancies and Show Syntax remain absent. Pixel styling is unverified because screenshot capture was unavailable. Cycle 56a stores a validated local Name and optional Identifier but never assigns labels; native Save/reopen was deliberately not exercised with a fake profile, and post-setup iA behavior remains unknown. The [iA native menu follow-up](../research/menu-audit-2026-09-24/native-submenus.md) captured previously hidden child labels, but screenshot-based, accessibility and remaining workflow acceptance are open. The Mac 0.2.0 RC1 download remains the Cycle 42 artifact. The [menu closeout plan](menu-closeout-plan.md) starts from the [screenshot audit](ia-menu-audit-2026-09-23.md); older cycle counts below are historical. Full iA Writer parity and hardware acceptance remain open.
