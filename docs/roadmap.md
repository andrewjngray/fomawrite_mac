...
**Current status — 24 September 2026:** Cycle 51 is the latest fully native-verified build. Current combined Cycle 57b/58a source passes 99 tests; the ordinary app is packaged from Cycle 53 and Dev is refreshed with that source. Partial native checks cover View/Go/Focus menus, fixture statistics, automatic small-root tags, completion/Undo, Custom and Fillers, the pre-setup Authors sheet, both bundled offline Help pages, local heading-fragment routing and Window → Center. Center was visible, enabled and invoked on synthetic Target.md without changing its status; the available-screen geometry is automated offscreen only, and OS/multi-display geometry remains unverified. A later live screenshot displayed the second `Same` heading with the caret at its start after Go → Open Link, but no screenshot file was persisted and preview scroll remains unverified. Cross-window fragment transfer and missing-anchor feedback remain open. Fragment support is bounded local navigation, not Markdown parser/output parity. Online Support and remaining Window/AppKit behavior also remain open. The [iA native menu follow-up](../research/menu-audit-2026-09-24/native-submenus.md) captured previously hidden child labels, but screenshot-based, accessibility and remaining workflow acceptance are open. The Mac 0.2.0 RC1 download remains the Cycle 42 artifact. The [menu closeout plan](menu-closeout-plan.md) starts from the [screenshot audit](ia-menu-audit-2026-09-23.md); older cycle counts below are historical. Full iA Writer parity and hardware acceptance remain open.
