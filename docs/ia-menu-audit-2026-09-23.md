...
| Transformations | **Partial (Cycle 50b)** | All four captured labels and order now appear under Edit → Transformations. Capitalize matched a synthetic iA `tEST of THE wORLD` → `Test Of The World` sample and one Undo restored the original. Omawrite's Make Title Case still uses the same algorithm; iA produced `tEST of the wORLD` on that sample, so exact Title Case behavior remains open. |
