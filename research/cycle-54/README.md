...
`./bin/build` and the full native-access `./bin/test` suite pass with **89 tests, zero failures and zero skips**. Focused tests cover the candidate rules, stale-caret refusal, Undo and a long-document time bound. The native popup, menu placement, keyboard traversal, dark/narrow appearance and IME behavior remain unverified because the Mac is locked and the Dev app cannot be closed normally. Use only [the synthetic sample](sample/Completion.md) when QA resumes.
