...
## Current integrated checkpoint: Cycle 59 with menu follow-ups

File and folder right-click menus include open/tab/window, info, favorites, duplicate/rename/Trash, Finder/sharing, output/clipboard, creation, sorting and view options. View → Template shares named presets across preview and export; Show Date offers Date Modified, Date Created and None. Format adds ordered tasks plus conservative Clear Styles. File/Edit follow the captured menu groups, and View offers Tree or current-folder List navigation and continuous/paginated preview routes. Cycles 52–58 add statistics and chrome modes, dynamic Go routes and bounded tag refresh, manual completions, live Custom/Fillers checks, Authors setup, bundled Help, Window Center and local heading-fragment navigation. After the Cycle 59 integrated checkpoint, Cycles 54b/54c added independent opt-in Smart Quotes and Smart Dashes subsets, and Cycle 50b–50c completed the captured Transformations labels and separated Capitalize from an observed English subset of Make Title Case. On macOS 27.0, **102 tests pass**. Both ordinary and Dev bundles were refreshed, locally ad-hoc signed and strict-verified. Dev verified Capitalize and Make Title Case with Undo in disposable drafts, then returned to a clean saved document; the ordinary bundle reopened its clean saved document. The integrated release remains an implemented subset, not complete iA parity or a full ten-menu state matrix. See the [current acceptance ledger](docs/release-acceptance.md), [build records](docs/build-cycles.md) and [menu audit](docs/ia-menu-audit-2026-09-23.md). The GitHub RC1 download remains the older Cycle 42 binary.
